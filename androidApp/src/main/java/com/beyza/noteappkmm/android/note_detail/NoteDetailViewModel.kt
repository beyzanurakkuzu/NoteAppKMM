package com.beyza.noteappkmm.android.note_detail
...
class NoteDetailViewModel @Inject constructor(
    ...
    val state = combine(
        noteTitle, isNoteTitleFocused,
        noteContent, isNoteContentFocused,
        noteColor
        ...
        NoteDetailState(
            ...
            noteColor = color as Long

        )
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), NoteDetailState())

    private val _hasNoteBeesSaved = MutableStateFlow(false)
    val hasNoteBeenSaved = _hasNoteBeesSaved.asStateFlow()
        ...
        savedStateHandle.get<Long>("noteId")?.let { existingNoteId ->
            ...
            viewModelScope.launch {
                noteDataSource.getNoteById(existingNoteId)?.let { note ->
                    ...
                    savedStateHandle["noteContent"] = note.context
        ...
        viewModelScope.launch {
            noteDataSource.insertNote(
                Note(
                    ...
                    context = noteContent.value,
                    colorHex = noteColor.value as Long,
                    created = DateTimeUtil.now()
                )
            )
            _hasNoteBeesSaved.value = true
        }
    }
}